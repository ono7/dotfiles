return {
  ...
  config = function()
    require("gen").prompts["Teach"] = {
      prompt = [[You are a helpful assistant. You specialize in software architecture and design patterns, Elaborate the following text:
...
$text"]],
    ...
    require("gen").prompts["Fix_Code"] = {
      prompt = "You are a helpful assistant. you specialize in software architecture and design patterns, Fix the following code. Only output the result in format ```$filetype\n...\n```:\n```$filetype\n$text\n```",
      replace = true,
      extract = "```$filetype\n(.-)```",
    }
    require("gen").prompts["DevOps"] = {
      prompt = "You are a helpful assistant. you specialize software architecture and design patterns. You offer help with cloud technologies like: Ansible, Golang, Docker, python. You answer with code examples when possible. $input:\n$text",
      replace = true,
    }
